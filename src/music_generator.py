    ...
    def tinynotation_v2(self, n_bars=10, per_measure=None):
        ...
        def generate_bar():
            ...
            while True:  # pick length that fits in bar
                init_length = np.random.choice(
                    self.time_range, p=self.time_weights)
            ...
            while bar_length < full_length[per_measure]:
                ...
                while new_note_idx >= len(self.note_range):
                    # reflect back
                    new_note_idx -= (new_note_idx-len(self.note_range))*2
                if new_note_idx < 0:
                    new_note_idx = 0
...
if __name__ == "__main__":
    ...
    generator = MusicGenerator(options=options)
    musc = generator.tinynotation_v2(n_bars=50, per_measure=4)

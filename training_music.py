...
all_crotchets_acci = converter.parse("tinynotation: 4/4 " + " ".join([a+"4" for a in fullrange_acci]))
all_halfs_acci = converter.parse("tinynotation: 4/4 " + " ".join([a+"2" for a in fullrange_acci]))
all_wholes_acci = converter.parse("tinynotation: 4/4 " + " ".join([a+"1" for a in fullrange_acci]))

single_quaver_range_acci = [a+"8 r8" for a in fullrange_acci]
single_semiquaver_range_acci = [a+"16 r16" for a in fullrange_acci]

all_single_quavers_acci = converter.parse("tinynotation: 4/4 " + " ".join(single_quaver_range_acci))
all_single_semiquavers_acci = converter.parse("tinynotation: 4/4 " + " ".join(single_semiquaver_range_acci))

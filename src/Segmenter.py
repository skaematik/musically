...
class Segmenter:
    ...
    def get_staff_lines_from_seeds(self, debug=False):
        ...
        sorted_lines = sorted(sorted_intra_lines,
                              key=lambda line: line[len(line) // 2][1])
        grouped_staff_lines = []
        current_line = 0
        current_staff = []
        prev_y_val = 0
    ...
    def getSymbols(self, merge_overlap=False):
        ...
        def boxes2symbols(boxes):
            ...
            for x, y, w, h in boxes:
                ...
                if line_num != -1:
                    symbols.append(Symbol(SymbolType.UNKNOWN, x, y, w,
                                          h, line_num, self.grouped_staff_lines[line_num]))
            return symbols
            ...
            for i in range(1, num_labels):
                for j in range(1, num_labels):
                    if i < j and i not in erased and j not in erased:
                        centroid_i, centroid_j = centroids[i,
                                                           :], centroids[j, :]
                        area_i, area_j = stats[i,
                                               cv2.CC_STAT_AREA], stats[j, cv2.CC_STAT_AREA]
                        x_i, x_j = stats[i,
                                         cv2.CC_STAT_LEFT], stats[j, cv2.CC_STAT_LEFT]
                        y_i, y_j = stats[i,
                                         cv2.CC_STAT_TOP], stats[j, cv2.CC_STAT_TOP]
                        w_i, w_j = stats[i, cv2.CC_STAT_WIDTH], stats[j,
                                                                      cv2.CC_STAT_WIDTH]
                        h_i, h_j = stats[i, cv2.CC_STAT_HEIGHT], stats[j,
                                                                       cv2.CC_STAT_HEIGHT]
                        ...
                        if centroid_dist < 30:
                            ...
                            if min_dist < 15:
                                ...
                                if h_i < 40 and h_j < 40:
                                    labels[labels == j] = i
                                    erased[j] = True
                                    stats[i, cv2.CC_STAT_LEFT], stats[i, cv2.CC_STAT_TOP], stats[i, cv2.CC_STAT_WIDTH], stats[i,
                                                                                                                              cv2.CC_STAT_HEIGHT] = merge_boxes((x_i, y_i, w_i, h_i), (x_j, y_j, w_j, h_j))
    ...
    def add_in_pictures(self, conncomps, from_staff_removed=True, fixed_width=True, width=300, final_width=150):
        ...
        for i in range(1, num_labels):
            ...
            sym = Symbol(SymbolType.UNKNOWN, x, y, w, h, line_num,
                         self.grouped_staff_lines[line_num])
            ...
            im = cv2.resize(im, (final_width, final_width),
        ...
        line_len = self.bin_img.shape[1]
        symbols = sorted(
            symbols, key=lambda sym: sym.line_num * line_len + sym.x)
        self.symbols = symbols
    ...
    def saveSymbols(self, format, save_origonal=False, path='./', width=150, reject_ratio=100, min_area=0,
        ...
        for sym in self.symbols:
            ...
            im = cv2.resize(im, (o_width, o_width),
                            interpolation=cv2.INTER_CUBIC)
            if sym.h / sym.w > reject_ratio:
                cv2.imwrite(os.path.join(
                    reject_path, 'ratio_' + format.format(count)), im)
            elif sym.w * sym.h < min_area:
                cv2.imwrite(os.path.join(
                    reject_path, 'size_' + format.format(count)), im)
            else:
                if dirty_times == 0:
                    ...
                    for i in range(dirty_times):
                        cv2.imwrite(os.path.join(
                            path, format.format(count)), dirty(im))
                        count += 1
            count += 1
    ...
    def blockout_markings(self):
        ...
        for y in range(3550, 3750):
            for x in range(300, 800):
                self.grey_img[y, x] = 255
                self.bin_img[y, x] = 255
        line_nums = [185, 465, 740, 1020, 1300,
                     1575, 1855, 2135, 2410, 2690, 2968]

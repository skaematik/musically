    ...
    def compare(self):
        ...
        while True:
            if i == len(xml_bars) or j == len(stream_bars):
                break
            bar_duration_err, bar_pitch_err, both_correct, bar_symbols, bar_pitched = compare_bar(xml_bars[i], stream_bars[j], i, j)
            if bar_duration_err > 1 and bar_pitch_err > 1:
                i += 1
                continue
            else:
                print("bar", i, j, "duration errors =", bar_duration_err, "pitch errors =", bar_pitch_err)
        ...
        print("both correct =", both_correct_count, "/", total_xml_symbols, "=", both_correct_count / total_xml_symbols * 100, "%")
...
def main():
    ...
    for filename in files:
        if filename.endswith('.jpg') or filename.endswith('.jpeg') or filename.endswith('.png'):
            print(filename)
            img = cv2.imread('./sheets/training/pieces/{}.png'.format(filename[:-4]))

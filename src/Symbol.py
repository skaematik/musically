...
class Symbol:
    ...
    def determine_pitch(self, templates, also_return_offsets=False, image=None):
        ...
        if (self.type == SymbolType.QUARTER or self.type == SymbolType.EIGHTH) and results[-1][2].startswith('half'):
            ...
            print('half mark')
        # print(results[-1])
        middle = self.y + ((results[-1][0][1] * 2) -
                           self.offsety) + results[-1][3][0]
        x_offset = self.x + \
            ((results[-1][0][0] * 2) - self.offsetx) + results[-1][3][1]
    ...
    def determine_beamed_pitch(self, templates):
        ...
        note, location, size, best_max = self.determine_pitch(
            templates, also_return_offsets=True, image=image)
        value = best_max
        image[location[1]:(location[1] + size[1]), location[0]              :(location[0] + size[0])] = 255
        result = [(note, location[0])]
        while value / best_max > 0.9:
            note, location, size, value = self.determine_pitch(
                templates, also_return_offsets=True, image=image)
            if value / best_max < 0.9:
                break
            image[location[1]:(location[1] + size[1]),
                  location[0]:(location[0] + size[0])] = 255
